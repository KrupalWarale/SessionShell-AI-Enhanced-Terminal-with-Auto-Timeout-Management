...
function Show-Banner {
    ...
    Write-ColoredText "+============================================================+" Blue
    Write-ColoredText "Timer: $script:IDLE_TIMEOUT seconds of inactivity" Yellow
...
function Show-Status {
    ...
    Write-Host "Timeout threshold: $($script:IDLE_TIMEOUT)s"
            ...
            if ($null -ne $keyPressed) {
                ...
                if ([int]$keyPressed -eq 13) {  # Enter key
                    ...
                    if (-not [string]::IsNullOrWhiteSpace($inputBuffer)) {
                        Write-Log "Executing command: $inputBuffer"
